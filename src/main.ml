...
let requirements_path =
  Filename.concat rando_path "SS Rando Logic - Requirements.yaml"

let glitchless_path =
  Filename.concat rando_path "SS Rando Logic - Glitchless Requirements.yaml"

let glitched_path =
  Filename.concat rando_path "SS Rando Logic - Glitched Requirements.yaml"
...
let eval map =
  ...
  let rec atom req =
    ...
    | exception Not_found ->
        ...
        |> Option.fold ~none:SOMETIMES ~some:(eval atom)
        |> fun b ->
        cache := StringMap.add req (Some b) !cache;
        b
  in
  eval atom
...
let rec flatten_expr = function
  ...
  | And (hd :: tl) -> (
      ...
      | (Or _ | Atom _) as r -> (
          match flatten_expr (And tl) with
          | Atom "Impossible" -> Atom "Impossible"
          | And l -> And (r :: l)
          | Atom "Nothing" -> r
          | s -> And [ r; s ]))
...
let prune map file =
  ...
  let prune_requirement r =
    {
      r with
      requirement =
        r.name |> Fun.flip StringMap.find map |> prune_exp |> flatten_expr;
    }
  in
  file
  ...
  |> List.filter (function
       | Com _ -> true
       | Req { requirement = Atom "Impossible"; _ } -> false
       | Req _ -> true)

let make_impossible name = StringMap.add name @@ Atom "Impossible"

let make_trivial name = StringMap.add name @@ Atom "Nothing"
...
let () =
  ...
  let file =
    ...
    with Lexer.Syntax_Error e ->
      Format.eprintf "%a" Location.print_location e;
      exit 1
  in
  close_in f;
  ...
  let glitched_map =
    req_map |> fun map ->
    List.fold_left (Fun.flip make_trivial) map glitchless_tricks
    |> make_trivial "BiT Trick"
    |> make_trivial "Early Thunderhead Trick"
  in

  let glitched_file = prune glitched_map file in
  let glitched_output =
    Format.formatter_of_out_channel @@ open_out glitched_path
  in
      ...
      (function
        | Req r
          when Str.string_match
                 (Str.regexp {|Can Set Scene Flag .x.*|})
                 r.name 0 ->
            Some r.name
        | _ -> None)
      file
  in
  let glitchless_map =
    req_map |> make_impossible "BiT Trick" |> fun map ->
    List.fold_left (Fun.flip make_impossible) map glitched_tricks |> fun map ->
    List.fold_left (Fun.flip make_impossible) map can_set_scene_flags
  in
  ...
  let glitchless_file =
    List.filter
      (function
        | Com c -> not @@ Str.string_match (Str.regexp {|# Can RBM .x.*|}) c 0
        | Req _ -> true)
      glitchless_file
  in
  let glitchless_output =
    Format.formatter_of_out_channel @@ open_out glitchless_path
  in
